...
class McGame:
    def __init__(self, difficulty=1, high_score=0):
        ...
        self.high_score_text = game_font.render(
            "HIGH: {}".format(self.high_score), False, INTERFACE_SEC
        )
    ...
    def draw(self, screen, defence):
        ...
        pygame.draw.line(
            screen, INTERFACE_PRI, [0, SKY_LEVEL], [SCREENSIZE[0], SKY_LEVEL], 2
        )
        pygame.draw.rect(
            screen,
            INTERFACE_PRI,
            (0, SCREENSIZE[1] - GROUND_LEVEL, SCREENSIZE[0], SCREENSIZE[1]),
        )
        score_text = game_font.render(
            "SCORE: {}".format(self.player_score), False, INTERFACE_SEC
        )
        screen.blit(score_text, (5, 10))
        screen.blit(self.high_score_text, (self.high_score_text_pos, 10))
        ammo_text = game_font.render(
            "AMMO: {}".format(defence.get_ammo()), False, INTERFACE_SEC
        )
    ...
    def update(self, missile_list, explosion_list, city_list):
        ...
        if (
            self.missile_frequency % self.missile_interval == 0
            and self.missile_count < self.max_missile_count
        ):
    ...
    def new_level(self, screen, defence):
        ...
        new_level = game_font.render(
            "NEW INBOUND MISSILES DETECTED", False, INTERFACE_SEC
        )
        get_ready = game_font.render("GET READY", False, INTERFACE_SEC)
        new_level_pos = (
            SCREENSIZE[0] // 2 - (new_level.get_width() // 2),
            SCREENSIZE[1] // 2 - (new_level.get_height() // 2),
        )
        get_ready_pos = (
            SCREENSIZE[0] // 2 - (get_ready.get_width() // 2),
    ...
    def game_over(self, screen):
        # display prompt for next level and give short pause
        game_over_msg = game_font.render(
            "YOU'RE CITIES HAVE BEEN ANNIHILATED", False, INTERFACE_SEC
        )
        score_msg = game_font.render(
            "SCORE: {}".format(self.player_score), False, INTERFACE_SEC
        )
        game_over_msg_pos = (
            SCREENSIZE[0] // 2 - (game_over_msg.get_width() // 2),
            SCREENSIZE[1] // 2 - (game_over_msg.get_height() // 2),
        )
        score_msg_pos = (
            SCREENSIZE[0] // 2 - (score_msg.get_width() // 2),
            SCREENSIZE[1] // 2
            - (score_msg.get_height() // 2)
            + game_over_msg.get_height(),
        )
