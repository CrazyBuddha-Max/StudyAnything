...
def exit_game(screen):
    ...
    exit_msg_pos = (
        SCREENSIZE[0] // 2 - (exit_msg.get_width() // 2),
        SCREENSIZE[1] // 2 - (exit_msg.get_height() // 2),
    )

    question_msg_pos = (
        SCREENSIZE[0] // 2 - (question_msg.get_width() // 2),
        SCREENSIZE[1] // 2 - (question_msg.get_height() // 2) + exit_msg.get_height(),
    )

    confirm_msg_pos = (
        SCREENSIZE[0] // 2 - (confirm_msg.get_width() // 2),
        SCREENSIZE[1] // 2
        - (confirm_msg.get_height() // 2)
        + exit_msg.get_height()
        + question_msg.get_height(),
    )
...
def pause_game(screen):
    ...
    pause_msg_pos = (
        SCREENSIZE[0] // 2 - (pause_msg.get_width() // 2),
...
def check_collisions(missile_list, explosion_list, city_list):
    ...
    for explosion in explosion_list:
        for missile in missile_list[:]:
            if explosion.get_radius() > distance(
                explosion.get_center(), missile.get_pos()
            ):
                score += missile.get_points() * explosion.get_points_multiplier()
                missile_list.remove(missile)
        for city in city_list[:]:
            if explosion.get_radius() > distance(
                explosion.get_center(), city.get_pos()
            ):
                city.set_destroyed(
                    True
...
def show_high_scores(screen, high_scores):
    ...
    wide_score = 0
    high_score_heading_pos = (
        SCREENSIZE[0] // 2 - (high_score_heading.get_width() // 2),
    ...
    for pos, record in high_scores.items():
        ...
        if wide_score <= len(record["score"]):
            ...
            record["score"] = (" " * (wide_score - len(record["score"]))) + record[
                "score"
            ]
        score_text = game_font.render(
            pos + " " + record["name"] + " " + record["score"], False, INTERFACE_SEC
        )
        score_text_pos = (
            SCREENSIZE[0] // 2 - (score_text.get_width() // 2),
            SCREENSIZE[1] // 2 - (text_height * text_y_pos_multiplier),
        )
    ...
    high_score_msg_pos = (
        SCREENSIZE[0] // 2 - (high_score_msg.get_width() // 2),
        SCREENSIZE[1] // 2 - (text_height * text_y_pos_multiplier),
    )
