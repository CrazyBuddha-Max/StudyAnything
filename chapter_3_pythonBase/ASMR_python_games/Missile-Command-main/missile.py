...
class Missile:
    def __init__(
        ...
        speed=1,
        points=10,
        trail_color=WARHEAD_TRAIL,
        warhead_color=WARHEAD,
    ):
        ...
        )  # full distance to target position
    ...
    def draw(self, screen):
        ...
        pygame.draw.line(
            screen, self.trail_color, self.origin_pos, self.pos, self.trail_width
        )
        # draw the warhead
    ...
    def update(self, explosion_list):
        if not self.detonated:
            self.pos = (
                self.origin_pos[0]
                + int(self.travel_dist * math.sin(self.angle) * self.incoming),
                self.origin_pos[1]
                + int(self.travel_dist * math.cos(self.angle) * self.incoming),
            )
    ...
    def explode(self, explosion_list):
        ...
        explosion_list.append(
            Explosion(self.pos, points_multiplier, explosion_radius, explosion_color)
        )
